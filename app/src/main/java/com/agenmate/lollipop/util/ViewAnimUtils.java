...
import com.agenmate.lollipop.ui.layout.RevealViewGroup;
...
public final class ViewAnimUtils {
  ...
  public static Animator createCircularReveal(View view, int centerX, int centerY,
    ...
    CircularRevealManager rm = viewGroup.getViewRevealManager();
    ...
    CircularRevealManager.RevealValues viewData = new CircularRevealManager.RevealValues(view, centerX, centerY, startRadius, endRadius);
    ObjectAnimator animator = rm.createAnimator(viewData);

    if (layerType != view.getLayerType()) {
      animator.addListener(new CircularRevealManager.ChangeViewLayerTypeAdapter(viewData, layerType));
    }
    return animator;
  }
}
